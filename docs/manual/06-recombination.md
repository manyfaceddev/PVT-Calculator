# Chapter 6 - Recombination / Live Oil Preparation
...
## 6.2 Two routes, and when to use each

### 6.2.1 Volumetric SF/FF (Carlsen & Whitson)
...
$$Rp_{total,cc} = \sum_{stages} R_{stage,cc} + FF_{cc} \quad \text{[cc gas std / cc STO]} \tag{6.2}$$
...
$$factor_{recomb} = \frac{P_{std}}{P_{recomb}} \times \frac{T_{recomb,R}}{T_{std,R}} \times Z_{recomb} \tag{6.3}$$

The oil-volume conversion factor is case-dependent:

$$Bo_{sep,eff} = \begin{cases} 1/SF & \text{Case 1} \\ 1.0 & \text{Case 2} \end{cases} \tag{6.4}$$

Cylinder mix ratio - cc gas at recombination conditions per cc oil charged:

$$CMR = \frac{Rp_{total,cc} \times factor_{recomb}}{Bo_{sep,eff}} \tag{6.5}$$
...
$$V_{oil,sep} = \frac{V_{live}}{1 + CMR} \tag{6.6}$$

$$V_{oil,STO} = \begin{cases} V_{oil,sep} \times SF & \text{Case 1} \\ V_{oil,sep} & \text{Case 2 (already STO)} \end{cases} \tag{6.7}$$
...
$$V_{oil,charge} = V_{oil,sep} \times \exp\!\big(c_o \times (P_{recomb} - p_{charge})\big) \tag{6.8}$$
...
$$c_o(P) = a_0 + a_1 P + a_2 P^2 + a_3 P^3 + \dots \tag{6.9}$$
...
$$V_{gas,std} = R_{stage,cc} \times V_{oil,STO} \tag{6.10}$$

$$V_{gas,sep} = V_{gas,std} \times \frac{P_{std}}{P_{stage}} \times \frac{T_{stage,R}}{T_{std,R}} \times Z_{stage} \tag{6.11}$$

$$V_{gas,recomb} = V_{gas,std} \times factor_{recomb} \tag{6.12}$$
...
GOR basis conversion (see Section 6.6 for the direction this divides in):

$$GOR_{eff} = \begin{cases} GOR / SF & \text{basis = SEPARATOR} \\ GOR & \text{basis = STOCK\_TANK} \end{cases} \tag{6.13}$$

Converted to cc/cc at standard conditions:

$$GOR_{cc} = GOR_{eff} \times 0.178108 \quad (= C_{scf}/C_{STB}) \tag{6.14}$$

Moles of gas per cc of stock-tank oil - ideal gas, PV = ZnRT solved for $n$
at standard conditions:

$$n_{gas} = \frac{P_{std} \times GOR_{cc}}{Z_{std} \times R \times T_{std}} \tag{6.15}$$

Moles of stock-tank oil per cc of stock-tank oil:

$$n_{oil} = \frac{\rho_{STO}}{MW_{STO}} \tag{6.16}$$

Gas/oil mole fractions of the wellstream:

$$f_{gas} = \frac{n_{gas}}{n_{gas}+n_{oil}} \qquad f_{oil} = \frac{n_{oil}}{n_{gas}+n_{oil}} \tag{6.17}$$

Mass fractions and wellstream MW:

$$MW_{wellstream} = f_{gas} \cdot MW_{gas} + f_{oil} \cdot MW_{STO} \qquad w_{gas} = \frac{f_{gas} \cdot MW_{gas}}{MW_{wellstream}} \qquad w_{oil} = \frac{f_{oil} \cdot MW_{STO}}{MW_{wellstream}} \tag{6.18}$$

Wellstream composition blend - mole-fraction-weighted, on each stream's own
normalized mol% basis:

$$z_i = f_{gas} \cdot y_i + f_{oil} \cdot x_i \tag{6.19}$$
...
|---|---|---|---|---|
| (6.14) | $GOR_{cc}$ | 60.378 | cc/cc | B25 |
| (6.15) | $n_{gas}$ | 0.00258036 | mol/cc STO | B26 |
| (6.16) | $n_{oil}$ | 0.00438162 | mol/cc STO | B27 |
| (6.17) | $f_{gas}$ | 0.370636 | - | B29 |
| (6.18) | $w_{gas}$ | 0.075937 | - | B31 |
| (6.18) | $MW_{wellstream}$ | 127.40 | g/mol | B33 |

Wellstream composition, eq. (6.19), golden-asserted spot values
...
## 6.3 Cylinder loading

`pvt.experiments.recombination.loading.plan_loading` takes a target
stock-tank-oil charge volume and the molar split (6.2.2) and works out how
...
$$V_{STO,equiv} = V_{oil,charge} \times \frac{\rho_{STO,load}}{\rho_{STO,60°F}} \tag{6.20}$$

Moles of oil charged and moles of gas required to hit the split's GOR:

$$n_{oil} = \frac{V_{STO,equiv} \times \rho_{STO,60°F}}{MW_{STO}} \qquad n_{gas} = split.n_{gas/cc,STO} \times V_{STO,equiv} \tag{6.21, 6.22}$$

$n_{gas}$ expressed as a standard-conditions gas volume:

$$V_{gas,std} = n_{gas} \times Z_{std} \times R \times \frac{T_{std}}{P_{std}} \tag{6.23}$$
...
$$P_{load,psia} = P_{load,psig} + 14.73 \tag{6.24}$$
...
$$factor_{load} = \frac{P_{load} \times Z_{std} \times T_{std}}{Z_{load} \times T_{load} \times P_{std}} \qquad V_{gas,charge} = \frac{V_{gas,std}}{factor_{load}} \tag{6.25, 6.26}$$

**Fits check** - total charge must be ≤ 95% of cylinder volume (headspace
reserve for thermal expansion during recombination):

$$total_{charge} = V_{oil,charge} + V_{gas,charge} \qquad fits = total_{charge} \le 0.95 \times V_{cylinder} \tag{6.27}$$
...
$Z=0.85$, $\rho_{STO,load}$ = 0.885 g/cc, against the 6.2.2 split above.

| Eq. | Quantity | Value | Unit | Workbook cell |
|---|---|---|---|---|
| - | $V_{oil,charge}$ | 150.0 | cc | B22 |
| (6.20) | $V_{STO,equiv}$ | 161.97 | cc | B23 |
| (6.21) | $n_{oil}$ | 0.709687 | mol | B25 |
| (6.22) | $n_{gas}$ | 0.417938 | mol | B29 |
| (6.23) | $V_{gas,std}$ | 9779.46 | cc | B30 |
| (6.25) | $factor_{load}$ | 385.39 | std cc/cc | B31 |
| (6.26) | $V_{gas,charge}$ | 25.38 | cc | B32 |
| (6.27) | `fits` / utilization | True / 17.5% | - | - |

## 6.4 Actual-GOR verification
...
$$n_{actual} = \frac{V_{gas,actual} \times P_{load}}{Z_{load} \times R \times T_{load}} \tag{6.28}$$

$$V_{std} = n_{actual} \times Z_{std} \times R \times \frac{T_{std}}{P_{std}} \tag{6.29}$$

$$V_{STO,actual} = V_{oil,actual} \times \frac{\rho_{STO,load}}{\rho_{STO,60°F}} \tag{6.30}$$

$$GOR_{actual} = \frac{V_{std}}{V_{STO,actual}} \times \frac{C_{STB}}{C_{scf}} \quad [\text{scf/STB}] \tag{6.31}$$

$$deviation\% = \frac{GOR_{actual} - GOR_{target}}{GOR_{target}} \times 100 \tag{6.32}$$
...
The plan (Section 6.3) called for charging **150 cc oil and 25.38 cc gas**.
...
| Eq. | Quantity | Value |
|---|---|---|
| (6.31) | $GOR_{actual}$ | 505.2 scf/STB (B47) |
| (6.32) | $deviation\%$ | +49.03% (B49) |
...
$$K_i = \frac{y_i}{x_i} \quad \text{for every } i \text{ with } x_i > 0 \tag{6.33}$$
...
This is the same y/x concept the Hoffman-Crump crossplot (Chapter 5, eq.
5.25) builds on for a flash gas/liquid pair. As of this writing, `k_values`
...
## 6.6 The D-018 open ruling - GOR basis direction
...
**What the engine does.** Eq. (6.13) above divides on the **opposite**
...
runs on the STO and gas streams' mol% bases (same eq. 5.23 as Chapter 5).
...
Actual-GOR verification (6.4) contributes its own `gor_actual_vs_target_pct`
...
- Molar route: `pvt.reporting.tables.recombination_tables` - **Molar
  Split** (9 rows, Section 6.2.2's outputs), **Loading Plan** (10 rows,
  Section 6.3's outputs plus fits/utilization), **QC Summary** (one row per
...
subcommand for the Molar/LiveOil-workbook route** - unlike Chapter 5's
